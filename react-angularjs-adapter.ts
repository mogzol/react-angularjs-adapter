...
export function angular2react<Props extends Record<string, unknown>>(
    ...
    return function Component(props: Props) {
        ...
        const portalsRef = React.useRef(new Map<number, { target: HTMLElement; content: React.ReactNode }>());
        const [, forceRerender] = React.useReducer((x) => x + 1, 0);

        const scope = React.useMemo<Scope<Props>>(() => getInjector().get('$rootScope').$new(true), []);
    ...
    return {
        ...
        controller: [
            ...
            class implements angular.IController {
                ...
                constructor($element: angular.IAugmentedJQuery, ...services: unknown[]) {
                ...
                public $onInit() {
                    bindingNames.forEach((name) => {
                        (this.props as Record<keyof Props, unknown>)[name] = this[name as keyof this];
                    });
                }
                ...
                public $onChanges(changes: OnChanges<Partial<Props>>) {
                    ...
                    const reactElement = React.createElement(Class, { ...this.props, ...this.injectedProps } as Props);
...
function kebabCase(str: string) {
    return (
        str
            ...
            // Add dashes before capitals, preserve groups of capitals (except last if not followed by dash or end)
